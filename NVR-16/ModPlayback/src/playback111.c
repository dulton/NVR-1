...
//#ifdef CHIP_HISI3531
	...
	{
		nBufSize = 512*1024;//nBufSize = 4*256*1024;//csp modify 20121116
	}
#else
	nBufSize = 80 * 1024;
	if((1 << nChn) & ((SPlayBackManager*)(psPlaybackChnInfo->pManagerParent))->nChnMaskOfD1)
					...
					|| (psPlaybackChnInfo->nForward == -1 && psPlaybackChnInfo->nCurFrameStamp >= nRefPts))
				{
					...
					else//video//csp modify 20130415
					{
					...
					#endif
						{
							...
							//printf("before tl_vdec_write, len:%d, data:(%02x %02x %02x %02x %02x)\n", sVds.len, sVds.data[0], sVds.data[1], sVds.data[2], sVds.data[3], sVds.data[4]);
							int rett = 0;
							rett = tl_vdec_write(nPlayNo,&sVds);
							//printf("chn[%d] tl_vdec_write result:%d len:%d\n", nChn, rett, sVds.len);
			...
			if(!psPlaybackChnInfo->nMediaFrameReady)
			{
				...
				for(nRetry = 0; nRetry < 3; nRetry++)
				{
					if(psPlaybackChnInfo->pFile)
					{
						...
						//if(nMediaType == 0 && nChn == 0) printf("chn[%02d], MediaType:%02d, pts:%llu，nRefPts:%lld, nRealSize:%d\n", nChn, nMediaType, nPts/1000, nRefPts, nRealSize);
					...
					|| (psPlaybackChnInfo->nForward==-1 && psPlaybackChnInfo->nCurFrameStamp>=nRefPts))
				{
					...
					else//video
					{
					...
					#endif
						{
							...
							//printf("before tl_vdec_write, len:%d, line:%d\n", sVds.len, __LINE__);
							int rett = 0;
							rett = tl_vdec_write(nPlayNo,&sVds);
							//printf("chn[%d] tl_vdec_write result2:%d len:%d\n", nChn, rett, sVds.len);
							//printf("sVds.len = %u, sVds.pts = %llu, rett = %d\n", sVds.len, sVds.pts, rett);
						}
						else//csp modify
						{
