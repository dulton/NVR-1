...
//when in interleaved mode, it must receive packet itself (excep interleaved header part)
extern int RTCP_handle_packet1(Rtcp_t *rtcp,void *payload,int payload_size);

extern int RTCP_handle_packet2(Rtcp_t *rtcp,void *payload,int payload_size);

extern int RTCP_handle_packet_bufandnet(Rtcp_t *rtcp,void *payload,int payload_size,int received_size);//csp modify 20140221

extern int RTCP_process(Rtcp_t *rtcp);

#if RTSP_ENABLE_RTCP == TRUE
//	#define RTCP_HANDLE1 	RTCP_handle_packet
	#define RTCP_HANDLE2	RTCP_handle_packet2
#else
//	#define RTCP_HANDLE1		RTCP_handle_packet1//lcy modify
	#define RTCP_HANDLE2	RTCP_handle_packet2
#endif
