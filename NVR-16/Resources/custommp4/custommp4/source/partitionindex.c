...
BOOL is_partition_index_valid(FILE *fp)
{
	...
	if(file_len != sizeof(partition_index_header)+header.total_file_nums*(sizeof(file_use_info)+NFILE_PER_SECT*sizeof(segment_use_info)))//yaogang hdd
	{
...
int pic_indexfilecp(char *src,char *dst)
{
	...
	if(fp1 == NULL)
	{
		printf("%s fopen src(%s) failed, errno=%d, str=%s\n", 
			__func__, src, errno, strerror(errno));
	...
	if(fp2 == NULL)
	{
		printf("%s fopen dst(%s) failed, errno=%d, str=%s\n", 
			__func__, dst, errno, strerror(errno));
	...
	if(ret < 0)
	{
		printf("%s fseek src(%s) failed, errno=%d, str=%s\n",
			__func__, src, errno, strerror(errno));
	...
	if(filelen < 0)
	{
		printf("%s ftell src(%s) failed, errno=%d, str=%s\n",
			__func__, src, errno, strerror(errno));
	...
	ret = fseek(fp1,0,SEEK_SET);
	if(ret < 0)
	{
		printf("%s fseek2 src(%s) failed, errno=%d, str=%s\n",
			__func__, src, errno, strerror(errno));
	...
	while(remain > 0)
	{
		...
		//printf("real=%d\n", real);
		ret = fread(byData,real,1,fp1);
		if(ret <= 0)
		{
			printf("%s fread src(%s) failed, ret: %d\n", 
				__func__, src, ret);
		...
		ret = fwrite(byData,real,1,fp2);
		if(ret <= 0)
		{
			printf("%s fwrite dst(%s) failed, ret: %d\n", 
				__func__, dst, ret);
...
VERIFYEND://csp add 20090331	
	...
	ret = fread(&index->header,sizeof(index->header),1,index->index1);
	if (ret <= 0)
	{
		printf("%s fread header failed, errno=%d, str=%s\n", 
	...
	ret = fseek(index->index1,0,SEEK_SET);
	if (ret < 0)
	{
		printf("%s fseek index1 failed, errno=%d, str=%s\n", 
			__func__, errno, strerror(errno));
	...
	//yaogang modify 20141225
	ret = fread(&index->pic_header,sizeof(index->pic_header),1,index->pic_idx);
	if (ret <= 0)
	{
		printf("%s fread pic header failed, errno=%d, str=%s\n", 
			__func__, errno, strerror(errno));
	...
	if (ret < 0)
	{
		printf("%s fseek pic index failed, errno=%d, str=%s\n", 
			__func__, errno, strerror(errno));
	...
	ret = fread(index->cachedst,index->cachelen,1,index->index1);
	if (ret <= 0)
	{
		printf("%s fread cachedst failed, errno=%d, str=%s\n", 
			__func__, errno, strerror(errno));
	...
	if (ret < 0)
	{
		printf("%s fseek index1 2 failed, errno=%d, str=%s\n", 
			__func__, errno, strerror(errno));
...
long seek_to_segment(partition_index *index,partition_index_header *pHeader,int file_no,int sect_no)
{
	if(index->valid)
	{
		...
		long position = sizeof(partition_index_header) + pHeader->total_file_nums * sizeof(file_use_info) \
			+ (sizeof(segment_use_info) * NFILE_PER_SECT) * file_no \
			+  sizeof(segment_use_info) * sect_no;//yaogang hdd
		return position;
	}
	return 0;
}
...
#if 0
//yaogang modify 20170419
//注意调用前需要加锁lock_partition_index(index);
//0:normal  1:bad file
int is_bad_file(partition_index *index, u32 file_no)
{
	int ret = 0;
	bad_file_use_info bf_u_info;

	memset(&bf_u_info, 0, sizeof(bad_file_use_info));
	
	long position = sizeof(partition_index_header) 
		+ index->header.total_file_nums*(sizeof(file_use_info) + NFILE_PER_SECT*sizeof(segment_use_info))
		+ file_no * sizeof(bad_file_use_info);

	ret = fseek(index->index1, position, SEEK_SET);
	if (ret)
	{
		printf("error %s: fseek failed, errno=%d, errstr=%s\n", 
				__func__, errno, strerror(errno));
		return 0;
	}
	
	ret = fread(&bf_u_info, sizeof(bad_file_use_info), 1, index->index1);
	if (ret != 1)
	{
		printf("error %s: fread failed, errno=%d, errstr=%s\n", 
				__func__, errno, strerror(errno));
	...
	if (bf_u_info.b_bad)
	{
		printf("%s disk(mount point): %s, file%d bad\n",
			__func__, index->mountpath, file_no);
	}

	return bf_u_info.b_bad;
}

//注意调用前需要加锁lock_partition_index(index);
//0:success  other:failure
int mark_bad_file(partition_index *index, u32 file_no, u32 position, u32 time)
{
	int ret = 0;
	bad_file_use_info bf_u_info;
	...
	if (ret)
	{
		printf("error %s: fseek 1 failed, errno=%d, errstr=%s\n", 
				__func__, errno, strerror(errno));
		return 1;
	}
	
	ret = fwrite(&bf_u_info, sizeof(bad_file_use_info), 1, index->index1);
	if (ret != 1)
	{
		printf("error %s: fwrite 1 failed, errno=%d, errstr=%s\n", 
				__func__, errno, strerror(errno));
	...
	ret = fseek(index->index2, position, SEEK_SET);
	if (ret)
	{
		printf("error %s: fseek 2 failed, errno=%d, errstr=%s\n", 
				__func__, errno, strerror(errno));
		return 1;
	}
	
	ret = fwrite(&bf_u_info, sizeof(bad_file_use_info), 1, index->index2);
	if (ret != 1)
	{
		printf("error %s: fwrite 2 failed, errno=%d, errstr=%s\n", 
				__func__, errno, strerror(errno));
	...
	if(index->valid)
	{
		...
		if(index->header.full_file_nums < index->header.total_file_nums)
		{
			...
			if(p_chn_info->file_no < index->header.total_file_nums)//320G-->594//yaogang hdd
			{
				...
				//yaogang hdd
				if(p_chn_info->sect_nums < (NFILE_PER_SECT-1) 
					&& p_chn_info->end_time <= (u32)time(NULL) 
					&& s_u_info.end_position <= (SIZE_OF_FILE_VECTOR-2*SIZE_OF_RESERVED))
				{
					//csp modify 20130106
					//position = seek_to_segment(index, &index->header, p_chn_info->file_no, p_chn_info->sect_nums);
					//memcpy(&s_u_info, index->mapdst1 + position, sizeof(s_u_info));
	...
	if(index->valid)
	{
		...
		if(index->header.full_file_nums < index->header.total_file_nums)
		{
			...
			if(p_chn_info->file_no < index->header.total_file_nums)//320G-->594//yaogang hdd
			{
				...
				文件容器满的条件
				1、有坏道
				2、内部存储文件数量sect_nums >= NFILE_PER_SECT-1
				3、文件结束位置接近上限
				*/
				if (!is_bad_file(index, p_chn_info->file_no)
					&& p_chn_info->sect_nums < (NFILE_PER_SECT-1) 
					&& p_chn_info->end_time <= (u32)time(NULL) 
					&& s_u_info.end_position <= (SIZE_OF_FILE_VECTOR-2*SIZE_OF_RESERVED))
				{
					//csp modify 20130106
				...
				{
					memset(&f_u_info, 0, sizeof(file_use_info));
			...
			if(index->header.recorded_file_nums < index->header.total_file_nums)
			{
				...
				*sect_offset = 0;
				...
				if (is_bad_file(index, index->header.recorded_file_nums))
				{
					printf("%s disk(mount point): %s, file%d bad Incredible, recorded_file_nums\n",
						__func__, index->mountpath, index->header.recorded_file_nums);
				}
...
int get_index_picinfo(partition_index* index, u16 file_no, u32 pic_no, pic_use_info *pinfo, u8 need_lock)
{
	int ret;
	u32 pos;
...
int search_rec_snap(partition_index *index,search_param_t *search,recsnapinfo_t *snapinfo_buf,int max_nums,u8 disk_no,u8 ptn_no)
{
	...
	if(index->valid)
	{
		...
		{
			ret = get_index_fileinfo(index, i, &finfo, 1);
			if (ret)
			{
			...
			{
				ret = get_index_picinfo(index, i, j, &pinfo, 1);
				if (ret)
				{
					printf("%s: get_index_picinfo failed\n", __FUNCTION__);
					return -1;
				}
				//printf("%s:yg 4\n", __FUNCTION__);
				//时间比较
