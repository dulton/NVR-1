...
int RTSP_STREAM_init(RtspStream_t *s,const char *name) 
{
	...
	s->param = (void *) src;
	VLOG(VLOG_CRIT,"rtsp stream(%s) init success.",name);
	return RTSP_RET_OK;
}
...
__RETRY:
    ret=fread(&frame,sizeof(Test264Frame_t),1,src);
    if (ret != 1) {
		RTSP_STREAM_reset(s);
		ret=fread(&frame,sizeof(Test264Frame_t),1,src);
		if(ret != 1){
...
int RTSP_STREAM_init(RtspStream_t *s,const char *name) 
{
	...
	VLOG(VLOG_CRIT,"rtsp stream(%s)(inspeed:%d) init success.",name,s->inspeed);
