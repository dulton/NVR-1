...
#define   II(a,   b,   c,   d,   x,   s,   ac)   {     (a)   +=   I   ((b),   (c),   (d))   +   (x)   +   (UINT4)(ac);     (a)   =   ROTATE_LEFT   ((a),   (s));     (a)   +=   (b);   }  
	...
	void   MD5_encode(char	*message,char	*buf)  
	{  
	  MD5_CTX	context;  
	  unsigned	 int   len	=	strlen(message); 
	  unsigned char temp[16];
	  MD5Init	(&context);  
	  MD5Update   (&context,  (unsigned  char *)message,len);  
	  MD5Final	 ((unsigned char *)temp,	&context);	

	sprintf(buf,"%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",temp[0],
...
int main(int argc,char ** argv)
{
	...
	int i = 0;
	rebuf = MD5_encode(message,rebuf);
	printf("md5:");
	while(i<16)
	{
