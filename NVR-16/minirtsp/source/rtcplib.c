...
static int rtcp_compute_jitter(Rtcp_t *rtcp,uint32_t sr_ts)
{
	...
	//VLOG(VLOG_CRIT,"jitter is: %d (dev:%d old:%d lastt:%d) ts(%#10x->%#10x)", //lcy modify
...
static void rtcp_compute_lost(Rtcp_t *rtcp,int srPs,int srOctect)
{
	...
	if(srPs != 0)//lcy modify
	{
		rtcp->rtp->fraction_lost = lost_ps*0xFF/srPs;
	}
	//VLOG(VLOG_CRIT,"comulative lost:%d fraction lost:%d lost_octet:%d",   //lcy modify
...
void rtcp_packet_interleaved_header(char *buffer,int chn,int size)
{
...
int rtcp_app_request(Rtcp_t *rtcp,char *buf_in,int *out_size)
{
	return 0;
}
...
static int rtcp_handle_sr(Rtcp_t *rtcp,char *buf)
{
	...
	//printf("receiver's SSRC = %#08x\n",rtcp->ssrc);//lcy modify
	//csp modify 20140221
	if(rtcp->src_id[0] == 0)
	{
		rtcp->src_id[0] = ssrc;
	}
	if(ssrc != rtcp->src_id[0] && rtcp->src_id[0] != 0)//lcy modify
	{
...
static int rtcp_handle_1st_sr(Rtcp_t *rtcp,char *buf)//lcy modify
{
	char *ptr = buf;
	rtcp_common_t *rtcpHeader=(rtcp_common_t *)buf;
	uint32_t ssrc;
	...
	// check ssrc
	ssrc = decode_int32be(ptr);
	printf("rtcp_handle_1st_sr error 0\n");
	rtcp->src_id[0] = ssrc;
	printf("rtcp_handle_1st_sr error 1\n");
...
static int RTCP_send_packet(Rtcp_t *rtcp,int bBye)
{
	...
	{
		ret = SOCK_sendto(rtcp->sock,rtcp->peername,dst_port,ps->buffer,size);
	}
	if(ret == RTSP_RET_FAIL)
	{
		rtcp->bAlive = false;
...
int RTCP_handle_packet(Rtcp_t *rtcp,void *payload,int payload_size)
{
	...
	if(rtcp->interleaved == true){
		...
		//csp modify
		//ret=SOCK_recvfrom(rtcp->sock,rtcp->peername,rtcp->chn_port_s,buf,sizeof(buf),0);
		ret=SOCK_recvfrom(rtcp->sock,rtcp->peername,&rtcp->chn_port_s,buf,sizeof(buf),0);
		if(ret == RTSP_RET_FAIL){
			return RTSP_RET_FAIL;
		}
...
int RTCP_handle_packet1(Rtcp_t *rtcp,void *payload,int payload_size)
{
	int ret,size;
	char buf[1024*16],*ptr=NULL,*pend=NULL;
	//RtspInterHeader_t *interHeader=NULL;
	rtcp_common_t *rtcpHeader=NULL;
	printf("RTCP_handle_packet1 error 000\n");//lcy
	if(rtcp == NULL){
		return RTSP_RET_FAIL;
	}
	
	if(rtcp->interleaved == true){
		ret = SOCK_recv2(rtcp->sock, buf, payload_size, 0);
		if(ret < 0){
			return RTSP_RET_FAIL;
		}
		ptr = buf;
		...
		printf("RTCP_handle_packet1 error 0\n");//lcy
		
		// if disable rtcp , return immediately with parse and response this packet
#if RTSP_ENABLE_RTCP == FALSE
		return RTSP_RET_OK;
#endif
	}else{
		// if disable rtcp , return immediately with parse and response this packet
		...
		//ret=SOCK_recvfrom(rtcp->sock,rtcp->peername,rtcp->chn_port_s,buf,sizeof(buf),0);
		ret=SOCK_recvfrom(rtcp->sock,rtcp->peername,&rtcp->chn_port_s,buf,sizeof(buf),0);
		printf("RTCP_handle_packet1 error 1\n");//lcy
		if(ret == RTSP_RET_FAIL){
			return RTSP_RET_FAIL;
		}
		ptr = buf;
	...
	pend = ptr+size;
	printf("RTCP_handle_packet1 error 2\n");//lcy
	do{
		rtcpHeader = (rtcp_common_t *)ptr;
		//
		//printf("RTCP Packet Type : %d\n",rtcpHeader->pt);
		if(rtcpHeader->pt == RTCP_PACKET_SR){
			ret=rtcp_handle_1st_sr(rtcp,ptr);
		}else{
			VLOG(VLOG_ERROR,"unknown rtcp packet type:%d",rtcpHeader->pt);
			ret = RTSP_RET_FAIL;
		}
...
int RTCP_handle_packet2(Rtcp_t *rtcp,void *payload,int payload_size)
{
	int ret,size;
	char buf[1024*16],*ptr=NULL,*pend=NULL;
	//RtspInterHeader_t *interHeader=NULL;
	...
	if(rtcp->interleaved == true){
		if(payload_size > 1024*16) return RTSP_RET_OK;//lcy modify
		ret = SOCK_recv2(rtcp->sock, buf, payload_size, 0);
		if(ret < 0){
			return RTSP_RET_FAIL;
		}
...
int RTCP_handle_packet_bufandnet(Rtcp_t *rtcp,void *payload,int payload_size,int received_size)
{
	int ret,size;
	char buf[1024*16],*ptr=NULL,*pend=NULL;
	rtcp_common_t *rtcpHeader=NULL;
	...
	if(payload_size > 1024*16)
	{
		printf("rtcp packet is too long,size:%d\n",payload_size);
		return RTSP_RET_OK;//lcy modify
	}
	
	memcpy(buf, payload, payload_size-uncompleted_size);
	if(uncompleted_size)
	{
		ret = SOCK_recv2(rtcp->sock, buf+payload_size-uncompleted_size, uncompleted_size, 0);
		if(ret < 0)
		{
			return RTSP_RET_FAIL;
		}
...
int RTCP_process(Rtcp_t *rtcp)
{
	...
	if(t_lasttime >= RTCP_MIN_TRANS_FREQ && rtcp->sr_pkt_got)
	{
	...
	Rtp_t *rtp) 
{
	...
	*r = rtcp;
	...
	{
		rtcp->sock = rtp_init_transport(cast_type,protocal,(role==RTSP_SERVER) ? chn_port_s : chn_port_c);
	}
...
int RTCP_destroy(Rtcp_t *rtcp)
{
	...
	{
		PORT_MANAGE_free_port((rtcp->role==RTSP_SERVER) ? rtcp->chn_port_s : rtcp->chn_port_c);
