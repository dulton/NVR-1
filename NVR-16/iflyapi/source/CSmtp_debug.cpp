...
void CSmtp::Send()
{
	...
	if(hSocket == INVALID_SOCKET)
	{
		...
		{
			printf("CSmtp::Send-2.1\n");
			
			throw ECSmtp(ECSmtp::WSA_INVALID_SOCKET);
			
			printf("CSmtp::Send-2.2\n");
		}
		
		printf("CSmtp::Send-2.3\n");
	}
								...
								const char *password/*=NULL*/)
{
	...
	try
	{
		printf("CSmtp::ConnectRemoteServer-1,nPort_=%d\n",nPort_);
		...
		printf("CSmtp::ConnectRemoteServer-2\n");
		...
		printf("CSmtp::ConnectRemoteServer-3\n");
		...
		printf("CSmtp::ConnectRemoteServer-4\n");
		...
		{
			return true;//此处应该是bug
		}
		
		printf("CSmtp::ConnectRemoteServer-5\n");
		...
		printf("CSmtp::ConnectRemoteServer-6\n");
		...
		printf("CSmtp::ConnectRemoteServer-7\n");
		
		if(authenticate && IsKeywordSupported(RecvBuf, "AUTH") == true)
		{
			printf("CSmtp::ConnectRemoteServer-8\n");
			...
			printf("CSmtp::ConnectRemoteServer-9\n");
			
			if(IsKeywordSupported(RecvBuf, "LOGIN") == true)
			{
				printf("CSmtp::ConnectRemoteServer-9.1,login:%s\n",m_sLogin.c_str());
				...
				printf("CSmtp::ConnectRemoteServer-9.1.1\n");
				...
				printf("CSmtp::ConnectRemoteServer-9.1.2\n");
				
				// send password:
				std::string encoded_password = base64_encode(reinterpret_cast<const unsigned char*>(m_sPassword.c_str()),m_sPassword.size());
				
				printf("CSmtp::ConnectRemoteServer-9.1.3\n");
				
				pEntry = FindCommandEntry(command_PASSWORD);
				
				printf("CSmtp::ConnectRemoteServer-9.1.4\n");
				
				sprintf(SendBuf,"%s\r\n",encoded_password.c_str());
				
				printf("CSmtp::ConnectRemoteServer-9.1.5\n");
				
				SendData(pEntry);
				
				printf("CSmtp::ConnectRemoteServer-9.1.6\n");
				
				ReceiveResponse(pEntry);
				
				printf("CSmtp::ConnectRemoteServer-9.2\n");
			}
			else if(IsKeywordSupported(RecvBuf, "PLAIN") == true)
			{
				printf("CSmtp::ConnectRemoteServer-9.3\n");
				...
				printf("CSmtp::ConnectRemoteServer-9.4\n");
			}
			else if(IsKeywordSupported(RecvBuf, "CRAM-MD5") == true)
			{
				printf("CSmtp::ConnectRemoteServer-9.5\n");
				...
				printf("CSmtp::ConnectRemoteServer-9.6\n");
			}
			else if(IsKeywordSupported(RecvBuf, "DIGEST-MD5") == true)
			{
				printf("CSmtp::ConnectRemoteServer-9.7\n");
				...
				if(getpeername(hSocket, (struct sockaddr*)&addr, &len) != 0)
				{
				...
				printf("CSmtp::ConnectRemoteServer-9.8\n");
			}
			else
			{
				printf("CSmtp::ConnectRemoteServer-9.9\n");
				
				throw ECSmtp(ECSmtp::LOGIN_NOT_SUPPORTED);
			}
			
			printf("CSmtp::ConnectRemoteServer-10\n");
		}
	}
	//catch(const ECSmtp&)
	catch(ECSmtp &e)
	{
		printf("CSmtp::ConnectRemoteServer-11\n");
		
		if(RecvBuf[0]=='5' && RecvBuf[1]=='3' && RecvBuf[2]=='0')
			m_bConnected=false;
		
		printf("CSmtp::ConnectRemoteServer-12\n");
		
		DisconnectRemoteServer();
		
		printf("CSmtp::ConnectRemoteServer-13\n");
		
		throw;
		
		printf("CSmtp::ConnectRemoteServer-14\n");
...
std::string ECSmtp::GetErrorText() const
{
	switch(ErrorCode)
	{
		...
		case ECSmtp::SELECT_TIMEOUT:
			return "Function select timeout";
		default:
...
void CSmtp::ReceiveResponse(Command_Entry* pEntry)
{
	...
	while(!bFinish)
	{
		printf("CSmtp::ReceiveResponse-1\n");
		
		ReceiveData(pEntry);
		
		//printf("CSmtp::ReceiveResponse-2,RecvBuf=%s\n",RecvBuf);
		
		line.append(RecvBuf);
		
		//printf("CSmtp::ReceiveResponse-3,RecvBuf=%s,line=%s\n",RecvBuf,line.c_str());
		
		size_t len = line.length();
		size_t begin = 0;
		size_t offset = 0;
		
		//printf("CSmtp::ReceiveResponse-4,RecvBuf=%s,line=%s,len=%d\n",RecvBuf,line.c_str(),len);
	...
	//printf("CSmtp::ReceiveResponse-7,line=%s\n",line.c_str());
	
	//strcpy_s(RecvBuf, BUFFER_SIZE, line.c_str());
	strcpy(RecvBuf, line.c_str());
	
	printf("CSmtp::ReceiveResponse-8\n");
	
	OutputDebugStringA(RecvBuf);
	
	printf("CSmtp::ReceiveResponse-9,reply_code=%d,valid_reply_code=%d\n",reply_code,pEntry->valid_reply_code);
	
	if(reply_code != pEntry->valid_reply_code)
	{
		printf("CSmtp::ReceiveResponse-10,error=%d\n",pEntry->error);
		
		throw ECSmtp(pEntry->error);
	}
	
	printf("CSmtp::ReceiveResponse-11\n");
}
