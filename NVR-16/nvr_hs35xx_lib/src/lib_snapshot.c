...
int tl_enc_snap(u8 EncChn, u16 *pWidth, u16 *pHeight, u8 *pdata, u32 *psize)
{
	...
	/******************************************
	step 1:  Create Venc Chn
	******************************************/	

	if (EncChn < 0 || EncChn >= ARG_VI_NUM_MAX)
	{
		printf("%s EncChn%d invalid\n", __func__, EncChn);
		return HI_FAILURE;
	}
	
	stSize.u32Width = *pWidth;
	stSize.u32Height = *pHeight;
	...
	******************************************/
	...
	if (HI_SUCCESS != s32Ret)
	{
		printf("%s HI_MPI_VENC_StartRecvPic EncChn%d faild with%#x!\n", __func__, EncChn, s32Ret);
		goto error1;
	}
	...
	******************************************/
	...
	s32Ret = HI_MPI_VPSS_GetGrpFrame(EncChn,  &stFrame, 0);
	if (HI_SUCCESS != s32Ret)
	{
		printf("%s HI_MPI_VPSS_GetGrpFrame EncChn[%d] faild with %#x!\n", __func__, EncChn, s32Ret);
		goto error2;
	}
	
	//printf("%s chn%d, width: %d, height: %d, format: %d\n", 
	//	__func__, EncChn, stFrame.stVFrame.u32Width, stFrame.stVFrame.u32Height, stFrame.stVFrame.enVideoFormat);
	...
	******************************************/
	...
	if (HI_SUCCESS != s32Ret)
	{
		printf("%s HI_MPI_VENC_SendFrame EncChn%d faild with %#x!\n", __func__, EncChn, s32Ret);
		HI_MPI_VPSS_ReleaseGrpFrame(EncChn,  &stFrame);
		//return s32Ret;
		goto error3;
	}

	s32Ret = HI_MPI_VPSS_ReleaseGrpFrame(EncChn,  &stFrame);
	if (HI_SUCCESS != s32Ret)
	{
		printf("%s HI_MPI_VPSS_ReleaseGrpFrame EncChn%d faild with %#x!\n", __func__, EncChn, s32Ret);
		//return s32Ret;
		goto error3;
	}
	...
	******************************************/
	...
	if (s32VencFd < 0)
	{
		printf("%s HI_MPI_VENC_GetFd EncChn: %d faild with%#x!\n", __func__, EncChn, s32VencFd);
		//return HI_FAILURE;
		goto error3;
	}
	...
	if (s32Ret < 0) 
	{
		printf("%s snap select EncChn: %d failed!\n", __func__, EncChn);
		//return s32Ret;
		goto error3;
	}
	else if (0 == s32Ret) 
	{
		printf("%s snap EncChn: %d time out!, time: %u\n", __func__, EncChn, getTimeStamp());
		s32Ret = HI_MPI_VENC_Query(EncChn, &stStat);
		if (s32Ret == HI_SUCCESS)
		{
			printf("\t u32LeftPics: %d, u32LeftStreamBytes: %d, u32LeftStreamFrames: %d, u32CurPacks:%d\n", 
				stStat.u32LeftPics, stStat.u32LeftStreamBytes, stStat.u32LeftStreamBytes, stStat.u32LeftStreamBytes);
		}
		else
		{
	...
	else
	{
		if (FD_ISSET(s32VencFd, &read_fds))
		{
			...
			if (s32Ret != HI_SUCCESS)
			{
				printf("%s HI_MPI_VENC_Query EncChn: %d failed with %#x!\n", __func__, EncChn, s32Ret);
				//return s32Ret;
				goto error3;
			}
			
			if (MaxPacks < stStat.u32CurPacks)
			{
				if (MaxPacks == 0)
				{
				...
				stStream.pstPack = (VENC_PACK_S*)malloc(sizeof(VENC_PACK_S) * MaxPacks);
				if (NULL == stStream.pstPack)
				{
			...
			if (HI_SUCCESS != s32Ret)
			{
				printf("%s HI_MPI_VENC_GetStream EncChn: %d failed with %#x!\n", __func__, EncChn, s32Ret);
				//free(stStream.pstPack);
				//stStream.pstPack = NULL;
				//return s32Ret;
				goto error3;
			...
			//printf("%s MaxPacks: %u, MaxLen: %u\n", __func__, MaxPacks, MaxLen);
			
			#endif

			if (framelen > *psize)
			{
				printf("%s EncChn: %d, framelen %d > *psize %d\n", __func__, EncChn, framelen, *psize);
				*psize = 0;
			}
			else
			{
			...
			if (s32Ret)
			{
				printf("%s HI_MPI_VENC_ReleaseStream EncChn: %d failed with %#x!\n", __func__, EncChn, s32Ret);
				//free(stStream.pstPack);
				//stStream.pstPack = NULL;
				//return HI_FAILURE;
				goto error3;
...
int tl_enc_snap(u8 EncChn, u16 *pWidth, u16 *pHeight, u8 *pdata, u32 *psize)
{
	...
	/******************************************
	step 1:  HI_MPI_VPSS_GetGrpFrame
	******************************************/
	VIDEO_FRAME_INFO_S stFrame;//stVFrame
	s32Ret = HI_MPI_VPSS_GetGrpFrame(EncChn,  &stFrame, 0);
	if (HI_SUCCESS != s32Ret)
	...
	//指定的分辨率大于当前解码码流的分辨率
	if ((stFrame.stVFrame.u32Width < *pWidth)
		||(stFrame.stVFrame.u32Height < *pHeight))
	{
		printf("%s chn%d, param: width*height: %d*%d, stream: width*height: %d*%d\n", 
			__func__, EncChn, *pWidth, *pHeight,
			stFrame.stVFrame.u32Width, stFrame.stVFrame.u32Height);
		*pWidth =  stFrame.stVFrame.u32Width;
		*pHeight = stFrame.stVFrame.u32Height;
	}
	
	/******************************************
	step 2:  Create Venc Chn
	******************************************/		
	stSize.u32Width = *pWidth;
	stSize.u32Height = *pHeight;
	...
	******************************************/
	...
	{
		printf("%s EncChn: %d FD_ISSET failed!\n", __func__, EncChn);
		s32Ret = -1;
		goto error3;
	}

error4:
	 HI_MPI_VENC_ReleaseStream(EncChn, &stStream);
