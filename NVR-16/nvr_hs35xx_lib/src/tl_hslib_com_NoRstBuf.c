		...
		for (chn = 0; chn < ARG_VI_NUM_MAX*2; chn++)
		{
			...
			switch ( ChnStatus[chn] )
			{
				case StatusChnStart:
				{
					...
					printf("%s StatusChnWaitBuf chn%d\n", __func__, chn);
				} break;
				...
				case StatusChnWaitBuf:
				{
					...
					if (GetPreviewQueueFirstFramePTS(chn, &IpcPts, &LocalPts) == 0)
					{
						...
						if ((IpcPts + 400*1000)  < QueueLastIPCPtsChn[chn])//缓冲400ms OK
						{
							...
							printf("%s StatusChnPreview chn%d\n", __func__, chn);
