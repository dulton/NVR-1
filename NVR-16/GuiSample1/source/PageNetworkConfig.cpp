...
extern "C" int SendMsgToDaemon(my_daemon_param *pDaemon);

BOOL StartP2PService()
{
	//system("/home/killp2p");
	//system("/home/p2p");
	...
	daemon.proc_type = MY_DAEMON_PROCTYPE_P2P;
...
//csp modify 20150110
#ifdef HI3535
BOOL SendPlatformInfo()
{
	my_daemon_param daemon;
	memset(&daemon, 0, sizeof(daemon));
	daemon.magic_no = MY_DAEMON_MAGIC;
	daemon.proc_type = MY_DAEMON_PROCTYPE_PLATFORM;
	daemon.op_type = MY_DAEMON_OPTYPE_RESTART;
	strcpy(daemon.arg1, "Hi3535");
			...
			case 4:
			{
				...
				switch(curID)
				{
					...
					case 1:
					{
						...
						{
							ret |= bizData_GetMobStreamParaDefault( i, &pTabPara[i] );	
						}	
					...
					case 4:
					{
						...
						sParaTgtIns.nChn = 0;
						BizGetDefaultPara(&sParaTgtIns, &bizSysPara);
						
						pCheckBox4->SetValue(bizSysPara.P2PEnable?1:0);
					} break;
					#endif
				}
			...
			case 5:
			{
				...
				switch(curID)
				{
					case 0://网络配置
					{
						...
						msg.chn = 0;
						strcpy(msg.note, GetParsedString("&CfgPtn.Network"));
					...
					case 1://网络子码流
					{
						...
						msg.type = EM_DVR_PARAM_SAVE;
						msg.chn = 0;
						strcpy(msg.note, GetParsedString("&CfgPtn.MobStream"));
						strcat(msg.note, GetParsedString("&CfgPtn.ParamSave"));
						upload_sg(&msg);
						
						// set gui ctrls
						if(pCheckBox1->GetValue())//全部
						{
							for(int i=0; i<nChMax; i++)
							{
						...
						for(int i=0; i<nChMax; i++)
						{
							...
							{
								printf("%s: \n", __func__);
								printf("\t nVideoResolution: %u\n", pTabPara[i].nVideoResolution);
								printf("\t nFrameRate: %u\n", pTabPara[i].nFrameRate);
								printf("\t nBitRateType: %u\n", pTabPara[i].nBitRateType);
								printf("\t nPicLevel: %u\n", pTabPara[i].nPicLevel);
								printf("\t nBitRate: %u\n", pTabPara[i].nBitRate);
							}
							*/
							bizData_SaveMobStreamPara( i, &pTabPara[i] );
					...
					case 2://邮件
					{
						...
						msg.type = EM_DVR_PARAM_SAVE;
						msg.chn = 0;
						strcpy(msg.note, GetParsedString("&CfgPtn.Email"));
						strcat(msg.note, GetParsedString("&CfgPtn.ParamSave"));
						upload_sg(&msg);
					...
					case 3://更多配置
					{
						...
						msg.type = EM_DVR_PARAM_SAVE;
						msg.chn = 0;
						strcpy(msg.note, GetParsedString("&CfgPtn.OtherSettings"));
						strcat(msg.note, GetParsedString("&CfgPtn.ParamSave"));
						upload_sg(&msg);
					...
					case 4://云功能
					{
						...
						msg.type = EM_DVR_PARAM_SAVE;
						msg.chn = 0;
						strcpy(msg.note, GetParsedString("&CfgPtn.P2PCloud"));
						strcat(msg.note, GetParsedString("&CfgPtn.ParamSave"));
						upload_sg(&msg);
						...
						if(bizSysPara.P2PEnable != pCheckBox4->GetValue())
						{
							bizSysPara.P2PEnable = pCheckBox4->GetValue();
							
							BizSetPara(&sParaTgtIns, &bizSysPara);
							
							if(bizSysPara.P2PEnable)
							{
								pStatic4[6]->SetText("&CfgPtn.Started");
								
								if(m_p2ppara.registered == 'Y')
								{
									StartP2PService();
								}
							}
...
*/
	...
	GetMobResoList(resoList, &nResoReal, 10);
		...
		for(j=0; j<nRealRow; j++)
		{
			...
			switch(i)
			{
				...
				case 4:
				{
					...
					{
						//printf("bitRate AddString %s\n",bitRateList[k].strDisplay);
...
void CPageNetworkConfig::OnTest20()
{
	...
	for(int j=0; j<3; j++)
	{
		...
		//csp modify 20131024
		//if(strlen(szRcvBox) > 5)
		if(strlen(szRcvBox) > 5 && strstr(sMailIns.pToBox,"@") != NULL && i != 0)
		{
...
BOOL ConstructMsg(P2PMsg *pMsg, EmP2PEvent emEvent, void *data, int len)
{
	if(pMsg == NULL)
	{
		return FALSE;
	...
	memset(pMsg, 0, sizeof(P2PMsg));

	memcpy(pMsg->head, g_msg_head, sizeof(g_msg_head));
	memcpy(pMsg->tail, g_msg_tail, sizeof(g_msg_tail));

	pMsg->event = emEvent;

	if(data != NULL)
	{
		if(len > MSG_DATA_LEN || len < 0)
		{
			return FALSE;
		}
	...
	unsigned int temp = pMsg->event;
	for(i = 0; i < MSG_DATA_LEN; i++)
	{
		temp += pMsg->data[i];
	}
	pMsg->verify = (temp % 256);

	return TRUE;
}

BOOL VerifyMsg(P2PMsg *pMsg)
{
	if(pMsg == NULL)
	{
		return FALSE;
	...
	if(memcmp(pMsg->head, g_msg_head, sizeof(g_msg_head)) || memcmp(pMsg->tail, g_msg_tail, sizeof(g_msg_tail)))
	{
		return FALSE;
	}
...
static BOOL RefreshP2PParam(P2PParam *para)
{
	if(para == NULL)
	{
		return FALSE;
...
static BOOL SetP2PParam(P2PParam *para)
{
	if(para == NULL)
	{
		return FALSE;
	...
	mtd.start = 0;
	mtd.size = 0x20000;
	int ret1 = ioctl(fd, MEMERASE, (char*)&mtd);
	if(ret1)
	{
		printf("flash MEMERASE failed\n");
		close(fd);
		return FALSE;
	}
	
	int ret2 = write(fd, para, sizeof(P2PParam));
	if(ret2 != sizeof(P2PParam))
	{
...
static BOOL CreateP2PSNFile(P2PParam *para)
{
	if(para == NULL)
	{
		return FALSE;
	...
	int fd = open ("/root/uuidfile",O_CREAT | O_TRUNC | O_RDWR, 0666);
	if(fd == -1)
	{
		return FALSE;
	}
	
	int ret = write(fd, para->sn, strlen(para->sn));
	if(ret <= 0)
	{
		close(fd);
		return FALSE;
...
void CPageNetworkConfig::Netp2pregister()
{
	RegisterFlag = 1;
	p2pregister();
}
...
{
	int ThisFlag = RegisterFlag;
	RegisterFlag = 0;
	
	int i = 0;
	P2PParam p2ppara;
	InitP2PParam(&p2ppara);
	
	int s = socket(AF_INET, SOCK_STREAM, 0);
	if(s == -1)
	{
		perror("failed to socket");
		pStatic4[6]->SetText("&CfgPtn.RegisterFailed");
		return;
	}
	...
	if(ThisFlag)//xdc
	{
		...
		address.sin_addr.s_addr = inet_addr("115.28.16.77");//
		address.sin_port = htons(33344);
		//printf("115.28.16.77\n");
	}
	else
	{
		address.sin_family = AF_INET;
		address.sin_addr.s_addr = inet_addr("192.168.1.199");
		address.sin_port = htons(33333-1);
	}
	...
	EmP2PRegStatus status = P2P_REG_WAIT_SN;
	
	P2PMsg sndMsg;
	ConstructMsg(&sndMsg, P2P_GETSN_REQ, (void *)"xx", strlen("xx"));
	
	int ret = send_loop(s, (char *)&sndMsg, sizeof(sndMsg));
	if(ret != sizeof(sndMsg))
	{
	...
	while(1)
	{
		...
		ret = recv_loop(s, (char *)&rcvMsg, sizeof(rcvMsg));
		if(ret != sizeof(rcvMsg))
		{
		...
		switch(status)
		{
		case P2P_REG_WAIT_SN:
			{
				if(rcvMsg.event == P2P_ERRCODE_BUSYING)
				{
					if(++wait <= 3)
					{
						printf("continue......\n");
						status = P2P_REG_WAIT_SN;
						sleep(1);
					}
					else
					{
				...
				else if(rcvMsg.event == P2P_GETSN_RESP)
				{
					char *p = strstr((char *)rcvMsg.data, "#");
					if(p == NULL)
					{
						printf("data error......\n");
						err = 1;
					...
					*p = '\0';
					...
					char *sn = (char *)rcvMsg.data;
					char *mac = p+1;
					printf("sn:%s\n",sn);
					printf("mac:%s\n",mac);
					
					memset(p2ppara.sn, 0, sizeof(p2ppara.sn));
					memset(p2ppara.mac, 0, sizeof(p2ppara.mac));
					strcpy(p2ppara.sn, sn);
					strcpy(p2ppara.mac, mac);
					p2ppara.registered = 'Y';
		...
		if(err || status == P2P_REG_END)
		{
			break;
		}
		
		ret = send_loop(s, (char *)&sndMsg, sizeof(sndMsg));
		if(ret != sizeof(sndMsg))
		{
			perror("failed to send");
			err = 1;
	...
	pStatic4[4]->SetText((char *)p2ppara.sn);
	pStatic4[5]->SetText((char *)p2ppara.mac);
	pStatic4[6]->SetText("&CfgPtn.RegisterSucc");
	
	pButton4->Enable(FALSE);//
	pCheckBox4->Enable(TRUE);
	pButton5->Enable(FALSE);
	
	SetP2PParam(&p2ppara);
	modifymac((char *)p2ppara.mac);
	
	memcpy(&m_p2ppara, &p2ppara, sizeof(P2PParam));
	
	CreateP2PSNFile(&m_p2ppara);
...
{	
	...
	for(i=0; i<2; i++)
	{
		pTable4->GetTableRect(i,4,&tmpRt);
		pStaticP2PWeb[i] = CreateStatic(CRect(rtTmp.left+tmpRt.left+4, rtTmp.top+tmpRt.top, rtTmp.left+tmpRt.right, rtTmp.top+tmpRt.bottom), this, szP2PWeb[i]);
		
		items[4].SubItems[0].push_back(pStaticP2PWeb[i]);
	}
	...
	pTable4->GetTableRect(1,7,&tmpRt);
	pButton5 = CreateButton(CRect(rtTmp.left+tmpRt.left+2, rtTmp.top+tmpRt.top+2, 
								rtTmp.left+tmpRt.left+2+100, rtTmp.top+tmpRt.bottom-2), this, "&CfgPtn.NetRegister", (CTRLPROC)&CPageNetworkConfig::Netp2pregister, NULL, buttonNormalBmp);
	pButton5->SetBitmap(pBmp_button_normal, pBmp_button_select, pBmp_button_push, pBmp_button_normal);
	//csp modify 20150110
	//items[4].SubItems[0].push_back(pButton5);
	pButton5->Show(FALSE);
	...
	//InitP2PParam(&m_p2ppara);
	GetP2PParam(&m_p2ppara);
	...
	else
	{
		...
		pCheckBox4->Enable(FALSE);

		pButton5->Enable(TRUE);
		//StartP2PService();//
	}
...
void CPageNetworkConfig::LoadPara( int curID ) // 0 common netpara 1 stream para
{
	...
	if(curID != 1)
	{
		...
		char p2p_enable = bizSysPara.P2PEnable;//(m_p2ppara.enable == 'Y')?1:0;
		pCheckBox4->SetValue(p2p_enable);
		
		if(m_p2ppara.registered == 'Y')
		{
			pStatic4[4]->SetText((char *)m_p2ppara.sn);
			//pStatic4[5]->SetText((char *)m_p2ppara.mac);
			pStatic4[5]->SetText((char *)sPIns.MAC);
		...
		{
			pStatic4[4]->SetText("&CfgPtn.UnRegistered");
			pStatic4[5]->SetText((char *)sPIns.MAC);
			
			pStatic4[6]->SetText("&CfgPtn.UnRegistered");
		}
		#endif
	}
	else//子码流yaogang modify 20150402 手机码流
	{
		for(int i=0; i<nChMax; i++)
		{
			//yaogang modify 20150402 mobile stream
			bizData_GetMobStreamPara( i, &pTabPara[i] );	
